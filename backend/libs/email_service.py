...
def send_report_email(
    ...
    try:
        ...
        # Add JSON attachment
        json_report = json.dumps(analysis_result, indent=2)
        json_attachment = MIMEApplication(json_report.encode('utf-8'), _subtype='json')
        json_attachment.add_header(
            'Content-Disposition', 
            'attachment', 
            filename=f'{brand_name.lower().replace(" ", "_")}_ai_search_report.json'
        )
...
def generate_report_html(brand_name: str, analysis_result: Dict[Any, Any]) -> str:
    ...
    <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: white; padding: 25px; border-radius: 8px; border: 1px solid #e5e7eb; margin-bottom: 25px;">
            ...
            {f'<p style="color: #6b7280; font-style: italic; margin-top: 15px;">+ {suggestions_count - 5} more recommendations in the full report</p>' if suggestions_count > 5 else ''}
        </div>
        
        <div style="background: #f3f4f6; padding: 20px; border-radius: 8px; margin-bottom: 25px;">
            <h3 style="color: #111827; margin-top: 0;">📎 Attachments</h3>
            <p style="margin: 10px 0; color: #374151;">• Full detailed JSON report with all data and analysis</p>
        ...
        <div style="background: #111827; color: white; padding: 20px; border-radius: 8px; text-align: center;">
            ...
            <p style="margin: 10px 0; font-size: 12px; opacity: 0.6;">Report generated on {datetime.now().strftime("%B %d, %Y at %I:%M %p")}</p>
        </div>

...
export const SignInPage = ({ className }: SignInPageProps) => {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    if (email && !isSubmitting) {
      ...
      // Redirect directly to analysis page
              ...
              >
                    ...
                    >
                      ...
                      <motion.button
                        ...
                        className="w-full sm:w-auto bg-[#0CF2A0] text-[#111111] px-5 py-2 rounded-md text-sm font-semibold hover:bg-opacity-90 transition-colors duration-200 whitespace-nowrap shadow-sm hover:shadow-md flex-shrink-0 disabled:opacity-50"
                        whileHover={{ scale: 1.03, y: -1 }}
                        whileTap={{ scale: 0.97 }}
                        transition={{ type: "spring", stiffness: 400, damping: 15 }}
                      >
                        {isSubmitting ? "Processing..." : "Get Free Report (30s)"}
                      </motion.button>
                    </motion.form>
                  </motion.div>
            </div>
